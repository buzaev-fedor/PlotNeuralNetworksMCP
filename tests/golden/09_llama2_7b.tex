...
\node[block, fill=clrattention_alt!85, minimum width=3.8cm, minimum height=0.85cm, text=clrtext, above=0.4cm of n_4] (n_5) {Self-Attention};
...
\node[block, fill=clrattention_alt!85, minimum width=3.8cm, minimum height=0.85cm, text=clrtext, above=0.4cm of n_12] (n_13) {Self-Attention};
...
\draw[arrow] (add_18.north) -- (n_19.south);
...
\draw[arrow] (n_21.north) -- node[right, yshift=-2pt, font=\sffamily\scriptsize, text=clrtext] {Output} ([yshift=0.6cm]n_21.north);
...
\node[below=0.6cm of current bounding box.south, subtitle, text width=14cm, align=center] {\textbf{LLaMA-2-7B -- 32L -- SwiGLU -- RoPE}};
\end{tikzpicture}
\end{document}
