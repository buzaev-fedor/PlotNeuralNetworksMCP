...
def block_EmbeddingStack(
    ...
    result = [
        to_Embed(
            name=tok, d_model=d_model, offset=offset, to=f"({botton}-east)",
            width=w, height=h, depth=d, caption="Token Emb",
        ),
        to_connection(botton, tok),
        to_Embed(
...
def block_FourierLayer(
    ...
    return [
        to_SpectralConv(
            name=spectral, modes=modes, offset=offset, to=f"({botton}-east)",
            width=w, height=h, depth=d, caption="Spectral Conv",
        ),
        to_connection(botton, spectral),
        to_Conv(
