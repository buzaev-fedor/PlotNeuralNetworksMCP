...
def block_2ConvPool(
    ...
    return [
        to_ConvConvRelu(
            name=ccr_name, s_filer=s_filer, n_filer=(n_filer, n_filer),
...
def block_Res(
    ...
    if num < 3:
        raise ValueError(f"block_Res requires num >= 3, got {num}")
    h, d, w = size
